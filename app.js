...
var light_status_topic = "/xiaoguo/lightdata";
...
function initHost() {
    ...
    host.on("lightPowerUpdated", function(light) {
        ...
        client.publish(light_status_topic, JSON.stringify(light))
    });
...
var client = mqtt.connect('tcp://public.vantiq.cn', settings);

client.on("connect", function () {
    client.subscribe("/xiaoguo/lightcontrol");
    client.subscribe("/xiaoguo/lighthsb");
});
...
client.on("message", function (topic, message) {
    ...
    // client.end();
    if(topic === "/xiaoguo/lightcontrol") {
        console.log("going to control the light")
        var msg = JSON.parse(message);
        console.log("light id: " + msg.light_id);
        console.log("active: " + msg.active);
        light_id = msg.light_id;
        status = msg.active ? "on" : "off";

        var data = '{"id":"{0}","operation":"{1}"}'.format(light_id, status);
        console.log("data: "　+ data);
        host.onUserCommand("light.power", data)
    } else if (topic === "/xiaoguo/lighthsb") {
        var msg = JSON.parse(message);
        var light_id = msg.light_id
        var brightness = msg.brightness
        var hue = msg.hue
        var saturation = msg.saturation
